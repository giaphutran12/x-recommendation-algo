...
export class PreviouslyServedFilter implements Filter {
  readonly name = 'PreviouslyServedFilter';

  filter(query: FeedQuery, candidates: ScoredCandidate[]): ScoredCandidate[] {
    const servedSet = new Set(query.served_ids);
    const result = candidates.filter((c) => !servedSet.has(c.tweet.id));
