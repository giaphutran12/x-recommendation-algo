  ...
  return (
    <div>
      <div className="flex items-center gap-4 px-4 py-3">
        ...
        <div>
          <h1 className="text-[#e7e9ea] font-bold text-xl leading-tight">
            {user.display_name}
          </h1>
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${avatarColor}33 0%, ${avatarColor}11 50%, #16181c 100%)`,
        }}
        aria-hidden="true"
      />
      ...
      <div className="px-4 pt-3 pb-4 space-y-3">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Badge
              variant="outline"
              className={cn('text-[11px] rounded-full font-medium', badgeClass)}
            >
              {user.persona_type}
            </Badge>
          </div>
...
export default async function ProfilePage({ params }: ProfilePageProps) {
  ...
  // Fetch user's tweets and follow status in parallel
  ...
  console.log(
    `[PROFILE] @${username} — tweets=${tweets.length}, isFollowing=${isFollowing}`,
  );
